...
import { Eye, FileStack, GitGraph, Sparkles } from 'lucide-react'
...
export function ReviewMindmapsPage() {
  ...
  const selectedOutputNoteId = selectedArtifact?.output_note_id ?? null
  ...
  return (
    <div className="space-y-4">
      <section className="fabric-panel space-y-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-cloth-muted">展示 mindmap 产物列表，支持管理员发起生成，并对输出 Markdown 中的 Mermaid 代码块进行可视化预览。</p>
          </div>
          <PermissionButton
            allowed={isAdmin}
            reason="仅管理员可生成思维导图"
        ...
        {(mindmapsQuery.isError || generateMutation.isError || outputNoteQuery.isError) ? (
          <ErrorStateCard
            title="导图任务状态异常"
            description={String(
        ...
        {generateMutation.data ? (
          <div className="rounded-xl border border-cloth-success/40 bg-cloth-success/10 p-3 text-sm text-cloth-ink">
            已创建思维导图任务 #{generateMutation.data.job_id}，输出笔记 #{generateMutation.data.output_note_id}（{generateMutation.data.relative_path}）。
          </div>
        ) : null}
          ...
          <PermissionGate allowed={isAdmin} reason="仅管理员可提交思维导图生成请求">
            <section className="fabric-card space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm text-cloth-muted">支持 scope、note_ids、prompt_extra。保持与后端契约一致，不额外扩展字段。</p>
              </div>
          ...
          <section className="fabric-card space-y-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-cloth-muted">支持多选，适合作为一次聚合式 Mermaid 导图输入。</p>
              </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[360px_minmax(0,1.1fr)_minmax(0,1fr)]">
        <section className="fabric-panel min-h-[680px]">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              (mindmapsQuery.data ?? []).map((artifact) => {
                ...
                return (
                  <button
                    ...
                    className={`block w-full rounded-xl border p-4 text-left ${active ? 'border-cloth-accent/55 bg-white/85 shadow-panel fabric-glow' : 'border-cloth-line/70 bg-white/45 hover:bg-white/65'}`}
                  >
                    <div className="flex items-center justify-between gap-3">
